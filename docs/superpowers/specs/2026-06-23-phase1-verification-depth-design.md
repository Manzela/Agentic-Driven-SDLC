...
| 11 | **`post_tool_use_hook` deltas (§2.5) — CORRECTED:** fix `_real_wiring` (imports nonexistent `check_wiring`) to use `analyze`→`emit_wiring_items`, **but with caller-context** (analyze the changed file *plus* its repo callers, or only emit symbols dead across the full-repo index) — a bare changed-files-only call floods every defined symbol as a false "dead" finding; add `mypy`; `*`schedule the **18.2 oracle** (a finding reaches `additionalContext`); assert **still exit-0 non-block** | tests **[H]**, `post_tool_use_hook.py` **[H]** | 18 |
...
| 16 | Phase-1 integration + **red-team evasion suite** (§8 T1–T9, granularity, baseline-vs-new) | tests **[H]** | 26 |
