...
**EARS:** The primary navigation SHALL present only the wordmark — carrying the "Autonomous SDLC Platform" category-descriptor lockup, legible at ALL scroll states — and the destinations How it works, Proof, Manifesto, plus Writing ONLY when the /writing flag is enabled (Writing SHALL be absent from nav while the route is stubbed/flag-OFF, with no orphan link), plus a single soft Get notified action, and SHALL NOT contain any pricing, demo, contact-sales, sign-in, or purchase affordance; WHERE the nav has a scroll-condense behavior, every animation SHALL map to a real state (DS-08) — the behavior SHALL be pinned/justified to a verifiable state change or removed entirely, never a decorative loop.
**Acceptance criteria:**
1. The <nav> banner contains exactly the wordmark (with the "Autonomous SDLC Platform" category-descriptor lockup) plus the live destination links (How it works, Proof, Manifesto, and Writing only when its flag is enabled) and one Get notified control — and nothing matching a pricing/demo/contact-sales/sign-in/buy denylist; while /writing is flag-OFF, Writing is absent from nav (no orphan link).
2. A CI content-lint over nav markup fails if any denied affordance string appears.
3. The category-descriptor lockup remains present and legible at all scroll states (assertion at top and scrolled positions); any nav scroll-condense behavior maps to a registered DS-08 state (pinned/justified to a real state) or is absent — no decorative/looping nav animation ships.
**Depends on:** none
...
**EARS:** IF a request resolves to no known route or content, THEN the site SHALL return HTTP 404 with a branded not-found page that preserves the global navigation, carries the verbatim governing-invariant trust line (single-sourced from CONTENT-11), and offers links to EACH canonical primary destination; the page SHALL read as an error within three seconds without using alarm-red.
**Acceptance criteria:**
1. GET /no-such-page returns HTTP 404 and the response contains the branded not-found marker, the global <nav> banner, and the verbatim governing-invariant trust line matching the CONTENT-11 canonical string byte-for-byte (no inline literal).
2. The 404 page contains anchor links to each canonical primary destination (/, /how-it-works, /proof, /manifesto), each resolving to a live route.
3. The page legibly reads as an error (a not-found marker / error-toned heading present at first paint) and uses no alarm-red / --error styling for the chrome (assertion that the page reads as an error in 3s without alarm-red).
**Depends on:** IA-13, CONTENT-11
...
**EARS:** IF an unrecoverable server or rendering error occurs, THEN the site SHALL return HTTP 500 with a branded error page that leaks ZERO internals (no stack trace, environment detail, or internal identifier), reads as an error within three seconds without using alarm-red, and SHALL offer a path back to the home route.
**Acceptance criteria:**
1. A forced server error returns HTTP 500 and the rendered body matches no stack-trace/env-var/internal-id pattern (zero internals leaked).
2. The 500 page contains a link resolving to /.
3. The page legibly reads as an error (error-toned marker/heading present at first paint) and uses no alarm-red / --error styling for the chrome (assertion that it reads as an error in 3s without alarm-red).
**Depends on:** IA-13
...
**EARS:** The global footer SHALL provide secondary navigation to all canonical destinations, a single soft Get notified email capture, and — as its one trust note — the single verbatim governing-invariant sentence single-sourced from CONTENT-11 (replacing any generic trust/security note), and SHALL NOT display any "N/N verified" count or other quantitative claim that is not independently evidence-backed.
**Acceptance criteria:**
1. The <footer> contentinfo contains resolvable links to every canonical destination plus one Get notified capture, and its trust note is the verbatim governing-invariant sentence matching the CONTENT-11 canonical string byte-for-byte (single-sourced, no inline literal and no separate generic trust note).
2. Footer text contains no substring matching the value-agnostic denylist \b\d+\s*/\s*\d+\s+(formally\s+)?(verified|checked)\b.
**Depends on:** IA-03, CONTENT-06, CONTENT-11
...
**EARS:** /writing and /docs SHALL DEFAULT to flag-OFF at launch (route + nav entry fully absent, no orphan nav link, per PAGE-16/IA-03); WHERE a section is instead explicitly enabled-but-empty — /docs as a launch-time placeholder or /writing with no published entries — the site SHALL render a designed stub/empty state that states the section's intent, preserves global navigation, and offers the soft Get notified action, without fabricating content or implying availability.
**Acceptance criteria:**
1. At launch defaults the flagged route is fully absent (per PAGE-16); WHERE explicitly enabled-but-empty, the route returns 200 and renders the stub container with intent copy, the global nav, and a Get notified control.
2. The stub DOM contains no article/list-item content nodes and no availability claim text.
**Depends on:** IA-01, PAGE-16
...
### HOME-01 — Hero: antagonist kicker, thesis H1, and cinematic closed-loop centerpiece  ·  P0  ·  EARS: event
**User story:** As a first-time visitor, I want the hero to name the enemy at first glance, assert the product thesis as its dominant headline, and stay legible even if motion never loads, so that I feel the problem and grasp "delivery that proves itself" without depending on an animation.
**EARS:** WHEN the homepage finishes its first contentful paint, the Hero section SHALL render — in this visual hierarchy — the brand wordmark, an antagonist line present at first paint as a kicker ABOVE/AS a lead-in to the headline (the felt pain that the proving is unpaid, invisible, and entirely the builder's), the single oversized display headline carrying the spec-mandated product thesis (delivery that proves itself) as the dominant first-paint headline, a one-line subhead, the soft primary action (See how it works), and a mount-point for the closed-loop centerpiece; WHERE the hero copy deck references the powering stack, it SHALL render it at SUPPORTING visual weight (subordinate to the kicker, headline, and subhead), not as a hero shout; WHEN motion is permitted, the hero centerpiece SHALL run its cinematic full-loop traversal (intent → decompose → implement → verify → prove → gate) and SHALL NOT pre-mount to, freeze on, or open at the gate stage; WHILE the centerpiece asset is still loading, the Hero SHALL render a legible static labeled diagram of the loop (intent -> decompose -> implement -> verify -> prove -> gate); IF the centerpiece asset fails to load or is unsupported, THEN the Hero SHALL retain the static labeled diagram as the permanent fallback and SHALL NOT show a broken-asset or error state.
**Acceptance criteria:**
1. In server-rendered HTML (JS disabled), the Hero DOM contains: a wordmark node, an antagonist kicker element rendered above/as a lead-in to the headline (present at first paint, sourced from CONTENT-02's candidate matrix), exactly one `h1` display headline carrying the product thesis as the dominant first-paint headline, a subhead element, an anchor whose text is "See how it works" resolving to `/how-it-works` (route per IA-03), and a centerpiece mount-point element.
...
4. The hero centerpiece, when animated, begins its traversal at the intent stage and completes a full cinematic loop; an assertion confirms the hero does not open at, freeze on, or pre-mount the gate stage, and the `--proven` accent does NOT appear in the hero (no green is rationed to the hero; accent reserved for HOME-05/HOME-06). The hero accent surface stays ≤3–5% of first-viewport pixels and all monospace text is confined to `[data-artifact]` nodes.
5. WHERE the hero copy deck enumerates the powering stack, it renders at supporting weight and names Claude Code, Neon, Langfuse, Playwright, and Semgrep as the in-scope tools and references Temporal only as an OPTIONAL / roadmap (Phase-5) durable-execution substrate, never as a wired-in spine member (satisfying CONTENT-17); a layout assertion confirms the stack copy is subordinate to the kicker, headline, and subhead.
**Depends on:** DS-01, DS-07, DS-08, IA-03, LOOP-05, CONTENT-02, CONTENT-17
...
5. The Fragmented-stack section (ordinal 2) contains a "betrayal" SUB-BEAT — a quiet green-suite-lies micro-sequence grounded in market research §1 that walks one concrete scar in order (a component implemented → its tests pass green → it is merged → it was never wired into a live execution path → it surfaces in production owned by the builder). The sub-beat is asserted present as static DOM text within ordinal 2 (NOT a new top-level section: the manifest stays the contiguous integers 1..8), uses no alarm-red / `--error` styling (quiet artifact-reveal, framed as recognition not fear), and introduces no `--proven` accent.
...
**EARS:** WHEN the Fragmented-stack section enters the viewport, it SHALL render a dense grid of at least twelve real named tool/provider tiles drawn from the product corpus; WHILE the section is pinned and the user scrolls through its progress range, the grid SHALL animate as a deterministic, scroll-bound state machine that collapses the many tiles into a single consolidated engine element, each transition step mapped to a discrete scroll-progress value; WHEN the grid resolves into the consolidated engine, the engine SHALL carry a human-legible lead label ("One closed loop. Proof built in.") as the PRIMARY three-second label and the engineering spine string as a SUBORDINATED static-DOM secondary caption; the enabling-primitive line ("deterministic per-edit enforcement only became possible recently — command-type hooks that fail closed") SHALL be present as static copy; Temporal SHALL render OUTSIDE the consolidated engine at progress = 1.0 as an honest seam (optional durable-execution roadmap, not a member of the consolidated engine); IF the section uses scroll-pinning, THEN the pin SHALL release cleanly at the end of its progress range with no jump, overlap, or trapped scroll.
**Acceptance criteria:**
1. At least twelve real named tool/provider tiles are rendered from the content corpus and present as static DOM text (tile labels assertable with JS disabled); a count assertion confirms ≥12 distinct named tiles.
...
3. At progress = 1.0 the grid has resolved to a single consolidated engine element whose PRIMARY label is the human-legible lead "One closed loop. Proof built in." and whose subordinated secondary caption is the engineering spine string "the gate-chain + coverage-model spine (Stop hook + OPA/Conftest + GitHub ruleset over a default-unproven feature_list.json)" rendered as static DOM text; on pin release, document flow resumes with measured CLS ≤ 0.1 and no overlapping/trapped-scroll condition.
4. Scrubbing progress backward reverses the state machine through the same discrete steps in reverse order.
5. The lead label "One closed loop. Proof built in." and the subordinated spine caption (naming the Stop hook, OPA/Conftest, the GitHub ruleset, and the default-unproven feature_list.json) are both present as static DOM text, with the lead label asserted as the primary/dominant label and the spine string as the secondary caption (string + hierarchy assertion).
6. Temporal renders OUTSIDE the consolidated engine element at progress = 1.0 (an honest seam — never absorbed into the engine), marked optional/roadmap; and the enabling-primitive line "deterministic per-edit enforcement only became possible recently — command-type hooks that fail closed" is present as static DOM text. No `--proven` accent is used in this section (consolidation reads via lateral separation, not green).
**Depends on:** DS-08, DS-09, CONTENT-17
...
**EARS:** WHEN the Human-as-infrastructure section enters the viewport, it SHALL render the replace-table mapping each human role to its machine substitute (human-as-memory -> requirement-ID-tagged durable state; human-as-verifier -> fail-closed coverage model; human-as-scope-enforcer -> bounded task decomposition; human-as-auditor -> tamper-evident execution trace; human-as-integration-lead -> the integration baseline is discovered for you, up front; human-as-tool-selector -> pre-integrated opinionated stack); each row's copy SHALL be outcome-first (lead with what the builder stops carrying) and then name the mechanism; WHILE the section is in view, each row SHALL reveal as a substitution-grammar micro-interaction tied to scroll in which the machine-substitute TEXT replaces the human-role TEXT in place (a lateral text substitution, not a green-lit reveal); the replace-table SHALL be fully readable as a static two-column table when motion is unavailable.
**Acceptance criteria:**
1. The server-rendered HTML contains a two-column table (or semantically equivalent role=table) with exactly the six role->substitute pairs above, all text assertable with JS disabled; the human-as-integration-lead substitute cell reads in plain English ("the integration baseline is discovered for you, up front"), not the prior jargon phrasing (string assertion).
2. The six row copies are sourced from the content layer (CONTENT-06), not hardcoded in the component, and each row is outcome-first then mechanism (the leading clause names the relieved human burden before the mechanism).
...
4. The scroll-driven reveal is a substitution grammar — the machine-substitute text visibly replaces the human-role text on scroll (text-for-text substitution); it changes only presentation, no row's text content is reachable only mid-animation, and NO `--proven` accent is applied to any row (substitution carries the meaning, not green).
5. The human-as-verifier AND human-as-auditor substitute cells each name per-edit prevention — that a write is prevented before it lands — via the PreToolUse + Stop hook, stated qualitatively with no latency/timing figure; and wherever Claude Code hooks are named the hook honest-limit note (command-type, fail-closed; PostToolUse cannot undo an executed action; roster emerging/version-gated) renders as qualitative copy (string/presence assertion).
**Depends on:** DS-08, CONTENT-06

### HOME-05 — "The line is held": closed-loop climax where the gate is the arrival  ·  P0  ·  EARS: state
**User story:** As a visitor, I want the shared loop to move quickly through its early stages and then arrive at the gate as the climax — where an unproven item is held and that hold reads as the win — so that I experience fail-closed as the most reassuring moment, not a failure.
**EARS:** WHILE the Closed-loop-explained section is the active beat, the Homepage SHALL advance the shared loop centerpiece through its stages in canonical order (intent, decompose, implement, verify, prove, fail-closed gate — gate last) in lockstep with scroll progress, displaying a synchronized per-stage caption, AND SHALL compress stages 1–5 relative to the gate beat at approximately a 3:1 ratio so the gate occupies roughly forty percent of the beat and reads as an ARRIVAL rather than a sixth step; the visible section heading SHALL be the climax line "The line is held"; WHEN scroll progress reaches the fail-closed-gate stage, the section SHALL present the gate holding the line on an unproven item as a system-correct event in which the held item's fade to --text-faint/--pending is the PRIMARY signal, a narrow "gate: holding — verified" verdict chip carries the single reserved --proven accent (the FIRST green on the page), and the held item routes to a HANDOFF state rendered visibly DISTINCT from COMPLETE; the section SHALL render the canonical gate-stage caption verbatim from CONTENT-13; IF the centerpiece is in reduced-motion or static mode, THEN the section SHALL present the same six stages in canonical order and the gate-holds-the-line beat as a captioned static sequence in which the gate cell carries a mandatory 2× emphasis over the other stage cells.
**Acceptance criteria:**
1. The six stage captions exist in static DOM in canonical order (gate last) and are sourced from the content layer; the gate-verdict copy and the canonical gate-stage caption are imported verbatim from CONTENT-13 and not re-declared in HOME; the visible section heading reads "The line is held" (string assertion).
2. Stages 1–5 are compressed relative to the gate beat at approximately 3:1 (the gate beat occupies ~40% of the section's scroll/visual budget), verified by a layout/progress assertion that the gate is the arrival, not an equally-weighted sixth step.
3. The held item's fade to --text-faint/--pending is the primary signal (decorative/disabled-only token, never the canonical --error token); the narrow "gate: holding — verified" verdict chip is the only element carrying the reserved --proven accent and is the page's first green; the held item routes to a HANDOFF state whose rendered treatment/label is asserted DISTINCT from a COMPLETE state.
4. With reduced-motion active, all six stages and the gate-holds beat render in their final/resolved captioned form, and the static gate cell carries a mandatory 2× emphasis over the other stage cells (reduced-motion final-state + emphasis assertion passes).
5. No alarm-red / --error styling appears on the gate verdict in any state.
6. The rendered gate-stage caption equals the canonical CONTENT-13 string ("The Stop hook holds the line locally; OPA/Conftest runs a zero-evidence policy at merge; a GitHub ruleset makes both required.") byte-for-byte (no inline literal in HOME).
**Depends on:** DS-01, DS-08, LOOP-05, CONTENT-13

### HOME-06 — Proof-model beat: the auditor question, one evidence record, the second and final green  ·  P0  ·  EARS: event
**User story:** As a skeptical visitor, I want the section to open by asking — in one breath, about my own future self — whether I could prove what the agent did, then show me one real four-field evidence record answering it, so that I render the verdict myself rather than read a marketing tally.
**EARS:** WHEN the Proof-model section enters the viewport, it SHALL open with the north-star auditor question compressed to one breath and co-framed for the visitor's self-interest — "Could YOU, six months from now, prove which requirement this code satisfied — and that it was independently verified?" — as the section's lead; it SHALL render ONE live monospaced evidence record showing the four-field schema (test_file, test_name, output_hash, collected_at) for a single requirement, preceded by a plain-language framing anchor; it SHALL state the independent-verifier line qualitatively; and it SHALL present the governing invariant only as a subordinate caption (NOT as visitor-facing lead copy); WHILE a requirement transitions from pending to proven, the panel SHALL drive the illumination from the COMPLETENESS of the evidence fields — the requirement ID and its evidence fields illuminate to the reserved --proven accent only once all four fields are present — decoupled from scroll arrival; the evidence record SHALL be marked illustrative/representative unmissably and SHALL NOT assert any specific aggregate verification count.
**Acceptance criteria:**
1. The section's lead is the compressed, self-interest-co-framed auditor question "Could YOU, six months from now, prove which requirement this code satisfied — and that it was independently verified?" present as static DOM text and positioned first in the section (string + order assertion); a plain-language framing anchor precedes the single evidence record exposing the four fields test_file, test_name, output_hash, collected_at.
2. The evidence record renders inside `[data-artifact]` nodes (the only place monospace is permitted), is exactly ONE record on the homepage, and is labeled/marked unmissably as illustrative/representative sample data.
...
4. The pending->proven illumination is driven by evidence-field completeness (accent applies only when all four fields are present) and is decoupled from scroll arrival; the reserved --proven accent in this section is the SECOND and final green beat on the page (after HOME-05's verdict chip); with reduced-motion the record renders in its resolved (proven, illuminated) final state.
...
6. The phrase "governing invariant" does NOT appear as visitor-facing lead/heading copy; the invariant content (deterministic gates decide completeness from verifiable facts; model self-assessment informs, never gates) appears only as a subordinate caption (assertion that no lead/heading node carries the label "governing invariant").
**Depends on:** DS-01, PAGE-03, LOOP-05, CONTENT-06

### HOME-07 — The-leap beat: a static two-column ledger resolved on arrival  ·  P1  ·  EARS: event
**User story:** As a visitor, I want a still two-column ledger — what the engine removes beside what it guarantees, each under a labeled group header — that I can read instantly without a scroll animation, so that the contrast is structural rather than carried by color alone.
**EARS:** WHEN the The-leap section enters the viewport, it SHALL render a static two-column ledger resolved on arrival (NO scroll-driven animation) that contrasts what the engine removes (manual tool-selection, manual context-management, manual micro-steering, manual traceability) against what it guarantees, under two labeled group headers ("removed" and "guaranteed") so the contrast is structural and not color-alone; the guarantees SHALL be stated as complete noun phrases (evidence-backed output, coverage-verified delivery, proof-traced audit) and the eliminations SHALL drop the "zero" prefix; the section SHALL be fully legible as a plain two-part list.
**Acceptance criteria:**
1. Static DOM contains both columns under the labeled group headers "removed" and "guaranteed": four "removed" items and three "guaranteed" items, all text assertable with JS disabled; the ledger is resolved on arrival with no scroll-bound animation (assertion that no scroll-progress motion drives this section).
2. List copy is sourced from the content layer (CONTENT-06); the guarantee items are the complete noun phrases "evidence-backed output", "coverage-verified delivery", and "proof-traced audit" (string assertion), and the elimination items carry no "zero" prefix (denylist assertion that the items do not begin with "zero").
3. Group separation is structural (labeled headers), not color-alone; the resolved "guaranteed" column may carry --proven/--accent (DS-01) and the "removed" column uses muted/--text-faint tokens; accent coverage stays ≤3–5% of viewport pixels and --proven appears only on the resolved guaranteed column.
4. The section renders in its resolved two-part state by default (and identically under reduced-motion, since there is no motion to disable).
...
**EARS:** WHEN the Who-its-for section enters the viewport, it SHALL open with a pain hook BEFORE any domain name — "When an auditor asks what the agent did, a self-report is not an answer." — then describe the audience as regulated and safety-critical software delivery (e.g., aerospace, medical, finance, defense) framed editorially and outcome-before-mechanism (the stakes/relief named before the substrate that delivers it); it SHALL include a no-lock-in micro-line ("your evidence is portable: an open four-field record and requirement-ID tags, not a proprietary cage"); the section SHALL NOT render a customer logo-wall, pricing, demo CTA, or any lead-gen funnel element, SHALL NOT reference subdomains or sibling products, and SHALL NOT use the --proven accent; WHERE named customers do not yet exist, the section SHALL use the audience/criteria framing and reserve any logo-wall pattern as a future, evidence-gated slot.
**Acceptance criteria:**
1. The section opens with the pain hook "When an auditor asks what the agent did, a self-report is not an answer." present as static DOM text and positioned BEFORE the first domain name (string + order assertion); the editorial audience copy (sourced from CONTENT-06) and the named domains aerospace, medical, finance, defense follow, framed outcome-before-mechanism.
2. The section contains no pricing text, no "demo"/"book"/"contact sales" CTA, no `<form>` lead-gen element, no logo-wall image grid, and no link to any sibling-product subdomain; the --proven accent is absent from the section (assertion).
...
6. The no-lock-in micro-line "your evidence is portable: an open four-field record and requirement-ID tags, not a proprietary cage" is present as static DOM text (string assertion).
**Depends on:** CONTENT-06, IA-03

### HOME-09 — Soft-close that resolves the betrayal arc and points onward  ·  P1  ·  EARS: optional
**User story:** As a convinced visitor, I want the close to resolve the opening betrayal — answering the agent that "looks done" — with a clear primary path onward and only soft actions, so that I can engage without hitting a sales motion or a content gate.
**EARS:** WHEN the Soft-close section enters the viewport, it SHALL present a final manifesto-tone statement that anchors back to the Beat-1 antagonist and resolves the green-suite-lied betrayal arc (the proving the builder once carried is now the product's job), ground "why nobody did this" in the new enabling primitive (deterministic per-edit enforcement only recently became possible — command-type hooks that fail closed), and offer soft actions only with a clear priority: "See how it works" as the PRIMARY action and "Read the full argument" (the manifesto) as the SECONDARY action, plus an optional Get notified affordance that is locatable but subordinate; the close microcopy SHALL be honest and SHALL NOT imply a committed launch date; IF a Get notified affordance is present, THEN it SHALL be a single low-friction control whose submission, success, error, and already-subscribed states are all explicitly handled, and it SHALL NOT gate access to any site content; the Soft-close section SHALL NOT contain pricing, demo booking, or any hard sales motion, and SHALL NOT use the --proven accent.
**Acceptance criteria:**
1. Static DOM contains a primary "See how it works" link to `/how-it-works` and a secondary "Read the full argument" link to `/manifesto` (routes per IA-03), with the visual/markup hierarchy asserting "See how it works" is primary and "Read the full argument" is secondary; the closing copy (from CONTENT-06) anchors back to the Beat-1 antagonist and resolves the betrayal arc (presence assertion tying the close to the opening pain).
2. The close grounds "why nobody did this" in the enabling primitive (command-type hooks that fail closed / deterministic per-edit enforcement newly possible) as static DOM text (string/presence assertion).
3. The Get-notified control delegates to the PRIV-owned notify component, is locatable but subordinate to the two onward links, renders distinct assertable states for submitting, success, error, and already-subscribed, and its microcopy implies no committed launch date (denylist assertion).
4. No site content is gated behind notify submission; the section contains no pricing/demo/booking element and no --proven accent.
...
### LOOP-01 — Closed loop of six ordered, named stages with a motion layer separate from a stationary artifact panel  ·  P0  ·  EARS: event
**User story:** As a visitor, I want a single continuous closed loop of six named, ordered stages whose moving geometry is kept separate from a still copy/artifact panel beside it, so that I can read the autonomous SDLC as one self-returning cycle and read the supporting detail without chasing it around the animation.
**EARS:** WHEN the centerpiece mounts and motion is permitted, the ClosedLoop SHALL drive the active-stage indicator around the loop (intent → decompose → implement → verify → prove → gate, returning to origin) in canonical order at a pace that completes one full autonomous traversal in 8–14 seconds; the centerpiece SHALL be composed of a MOTION LAYER (loop geometry, the six node/stage labels, and the reserved accent only) and a STATIONARY adjacent artifact/copy panel that holds tool names, captions, and evidence detail; the motion layer SHALL NOT contain tool/provider names.
**Acceptance criteria:**
...
2. The active-stage indicator advances in canonical order and one full autonomous traversal measured from runtime telemetry falls within 8–14 seconds (inclusive) on the default playthrough.
3. The motion layer and the stationary artifact/copy panel are distinct DOM regions; an assertion confirms the motion layer carries only loop geometry, the six node/stage labels, and the reserved accent — and that no tool/provider name renders inside the motion layer (tool names live only in the stationary panel).
**Depends on:** DS-08
...
**EARS:** The ClosedLoop SHALL be driven by a single Rive state machine (one source of truth) whose named states correspond to the loop stages and whose named inputs and triggers are the only mechanism by which the host application changes what is displayed; the same state machine SHALL support exactly two playback modes selected by the host — a hero AUTONOMOUS mode (self-driven 8–14s traversal per LOOP-01) and a HOME-05 SCROLL-DRIVEN mode (progress bound to scroll) — with no second/duplicate state machine.
**Acceptance criteria:**
...
3. A single Rive state machine is the source of truth for both the hero autonomous traversal and the HOME-05 scroll-driven beat (single-source-of-truth assertion); the host selects between the autonomous and scroll-driven playback modes via named inputs, and no duplicate/second state machine exists in the build.
**Depends on:** DS-08
...
5. Every per-stage tool/provider label and substrate name renders in the STATIONARY artifact/copy panel (LOOP-01), never inside the motion layer (the motion layer holds only loop geometry, the six node labels, and the reserved accent); a region assertion confirms no tool/provider name appears in the motion layer.
...
**EARS:** The /manifesto content SHALL be authored in MDX as numbered sections whose §1 opens ANTAGONIST-FIRST with the felt pain (the proving the builder personally carries; the green suite that lied) and then states the category/integrator thesis as §1's second paragraph, proceeding through the fragmented-stack problem, human-as-infrastructure, the closed loop, and the proof wedge, and closing without a hard sell, and WHERE it states the governing invariant it SHALL quote it precisely: deterministic gates decide completeness from verifiable facts only; model self-assessment informs, never gates. WHERE the closed-loop section names the enforcement layer, it SHALL name it concretely — "The only thing that can halt or redirect a running agent is a deterministic Claude Code command hook (PreToolUse, PostToolUse, Stop, SubagentStop, PreCompact, SessionStart) — not a prompt, not CLAUDE.md — and command hooks fail closed." — and SHALL name the four bounded subagents (initializer, implementer, verifier, research). It SHALL also state the bounded-execution and self-correction note: iteration/token/no-progress caps route to HANDOFF (a terminal state distinct from COMPLETE), and the team found and fixed its own HANDOFF infinite-block defect (a self-audit trust signal). Claude Code hooks are command-type and fail closed; PostToolUse cannot undo an executed action; the hook roster is emerging/version-gated.
**Acceptance criteria:**
1. The MDX renders as sequentially numbered sections covering the five required spine elements (presence assertion per section id); §1's first paragraph is the felt-pain antagonist framing and the integrator/category thesis appears as §1's second paragraph (DOM order assertion within §1).
...
5. The held-item/HANDOFF verdict-chip copy is defined here exactly once — the "gate: holding — verified" chip string and the HANDOFF label (a terminal state distinct from COMPLETE) — and HOME-05, PAGE-04, and the loop requirements render it from this source (single-source assertion).
6. The style sheet records the green-rationing rule: the --proven/--signal accent may render as green ONLY at the gate-hold verdict chip (HOME-05) and the completed evidence record (HOME-06); a build assertion flags any green accent introduced in the hero (HOME-01), the fragmented-stack collapse (HOME-03), the replace-table (HOME-04), the leap's removed column (HOME-07), or the who-its-for/soft-close beats (HOME-08/HOME-09).
**Depends on:** DS-07
...
**EARS:** The site SHALL render a "Powered by — built on proven, in-production solutions" module positioned as the IMMEDIATE sequel to the HOME-03 fragmented-stack collapse — answering "what's in the box?" by showing the consolidated engine's components as the collapse reveal, NOT a logo-wall — presenting the stack as machine artifacts (no customer logos, no unverifiable stats) as the claims-integrity-safe substitute for a logo-wall; each manifest entry SHALL render the tool name (mono `[data-artifact]`) together with its B-block ID and a one-line human gloss / qualitative pain-point tie-in drawn from the claims registry (the B-block ID is retained but never stands alone — it is always paired with a plain-English one-line gloss). Where Claude Code hooks are named, the module SHALL co-state the hook honest-limit: Claude Code hooks are command-type and fail closed; PostToolUse cannot undo an executed action; the hook roster is emerging/version-gated.
**Acceptance criteria:**
1. The <PoweredBy> module renders each tool as a mono [data-artifact] entry sourced from the committed PRD stack manifest; every listed tool is asserted to be a member of that manifest; the module is positioned as the immediate sequel to the HOME-03 collapse (it follows the consolidated-engine reveal and frames the engine's components as "what's in the box", asserted by ordinal adjacency to HOME-03).
2. The module contains zero logo <img> nodes and zero numeric stat strings (DOM and numeric-scan assertion); the components are presented as the collapse reveal, not a logo parade.
3. Each manifest entry renders three parts: the tool name in a mono `[data-artifact]` node, its B-block ID, and a one-line human gloss / qualitative pain-point tie-in in plain English sourced from the claims registry (assertion that all three are present per entry, that the B-block ID never appears without its one-line gloss, and that no numeric stat appears in the gloss).
...
**EARS:** The site SHALL state the vendor-neutral schema (feature_list.json + EvidenceRecord + requirement-ID Baggage) and the "opinionated defaults, not a cage" no-lock-in promise, rendering the FULL TRIPLE (the schema naming, a concrete EvidenceRecord JSON glimpse, and the no-lock-in promise) on /how-it-works AND in the manifesto, and rendering a homepage MICRO-LINE only (the portable-evidence one-liner in HOME-08, not the full block); WHERE the concrete EvidenceRecord glimpse is shown it SHALL render a six-line JSON glimpse on a mono `[data-artifact]` node together with a one-line human gloss, and the gloss for "requirement-ID Baggage" SHALL explain it as standard distributed-tracing Baggage headers (no proprietary format), so the schema reads as portable rather than a cage.
**Acceptance criteria:**
1. A copy-presence check asserts the full no-lock-in triple — the schema naming (feature_list.json, EvidenceRecord, requirement-ID Baggage), a concrete six-line EvidenceRecord JSON glimpse on a mono `[data-artifact]` node, and the "opinionated defaults, not a cage" promise — renders on /how-it-works AND in the manifesto; the homepage carries only the HOME-08 portable-evidence micro-line, not the full block (assertion that the full triple is absent from the homepage).
2. The no-lock-in claim resolves to a claims-registry entry with a corpus citation (claims-ledger citation check passes).
3. The EvidenceRecord glimpse renders as a six-line JSON artifact on a `[data-artifact]` mono node with a one-line human gloss; the gloss for "requirement-ID Baggage" describes it as standard distributed-tracing Baggage headers (not a proprietary format) — string/presence assertion — and the glimpse contains no unverifiable verification count (denylist passes).
**Depends on:** CONTENT-06, IA-03
...
2. Each stage node contains a non-empty plain-language paragraph, a mechanism description, and a machine-artifact reference rendered with monospace styling only on a `[data-artifact]` node; each stage uses a two-beat reveal in which the plain-language outcome is present first and the mechanism/artifact detail is revealed on demand (assertion that the outcome beat precedes the mechanism/artifact beat and that mechanism detail is not all surfaced at once).
3. A build assertion verifies that each of the six stages names ≥1 mechanism from its controlled-vocabulary set (intent → a human brief → EARS-format requirement validated by the SMT spec compiler + spec_validator, with EARS named as the notation/format and NOT as a tool; decompose → initializer subagent + default-unproven feature_list.json; implement → implementer subagent + git worktree + ordered PreToolUse plan/scope gates; verify → verifier subagent (five layers) + SubagentStop evidence gate; prove → four-field Evidence_Record + hash-chained audit log + Z3 logic model; gate → Stop hook + OPA/Conftest + GitHub ruleset); the intent stage is asserted to describe EARS as the requirement notation/format (a human brief becoming an EARS-format requirement), not as a tool or step; the page also names the strict sequencing (the ordered PreToolUse gate chain, and that the Stop hook evaluates HANDOFF triggers before the unproven-items gate).
...
3. Every block is assigned to exactly one cluster, and the cluster set equals EXACTLY the six canonical loop stage labels (intent, decompose, implement, verify, prove, gate) — a set-equality assertion against the CONTENT-13 canonical six-stage order; the taxonomy is framed as "the same loop at higher resolution" (string/presence assertion), and no block is orphaned.
4. The blocks render in a two-tier hierarchy that visibly elevates B09, B12, B13, and B16 above the remaining blocks (hierarchy assertion); each block's one-line description names its signature mechanism (build assertion that the description is non-empty and mechanism-bearing); B09, B12, B13, and B16 match their canonical signature-mechanism strings (fail-closed completion gate: Stop hook + OPA/Conftest + GitHub ruleset; mid-flight steering: PreToolUse prevention + PostToolUse feedback; anti-loopmaxxing: iteration/token/no-progress caps routing to HANDOFF; security/control: SAST + secrets + SLSA + DAST + kill-switch).
...
**EARS:** The /proof page SHALL present, in order, the auditor test (the north-star question) FIRST as the challenge, then — as the answer — the governing invariant (deterministic gates decide completeness from verifiable facts only; model self-assessment informs but never gates), the four-field evidence schema (test_file, test_name, output_hash, collected_at) accompanied by a LIVE RENDERED evidence artifact, and the independent-verifier principle (the verifier has no write access and never grades its own output), each as a distinct titled section; the page SHALL keep the honest limits out loud. The page SHALL clarify qualitatively that the hash-chained log is tamper-evident proof that execution happened as recorded (proof of execution) — distinct from proof that generated code is correct (proof of correctness), which is out of scope — and that the Z3 logic model checks the requirement logic model, not generated code.
**Acceptance criteria:**
1. The page DOM contains four distinct titled sections with the auditor test (north-star question) FIRST as the challenge, followed by the governing invariant, the evidence schema, and the independent-verifier principle as the answer; assert by heading text and ordinal position (auditor test at ordinal 1).
2. The evidence-schema section lists exactly the four field names `test_file`, `test_name`, `output_hash`, `collected_at`, each on a `[data-artifact]` mono node and no other field present, and is accompanied by a live rendered evidence artifact (a concrete record rendered, not described); assert the artifact node is present on a `[data-artifact]` mono node.
...
**EARS:** The page SHALL render a bridge sentence directly ABOVE the interactive trace that orients the reader (what the trace shows and why the held gate is the win); WHEN a visitor activates the sample audit-trace component, the component SHALL render a monospaced, requirement-ID-tagged, hash-chained execution trace for a fictional-but-realistic feature and SHALL allow the visitor to advance one step at a time through states unproven → evidence-attached → proven and to a final fail-closed gate evaluation; the evidence-pending state SHALL be rendered in the `--pending` token; WHILE the trace is at the gate step with any item still unproven, the component SHALL show the gate holding (closed) in the `--proven` accent color WITH copy at first contact that reframes the green as "holding the line" (so the green reads as correct, not as a pass), route the held item to a HANDOFF state rendered DISTINCT from COMPLETE, and SHALL NOT mark the run COMPLETE.
**Acceptance criteria:**
1. A bridge sentence is present directly above the interactive trace (DOM order assertion) orienting the reader to the trace and the held-gate-as-win framing.
2. Activating the component then advancing emits trace rows tagged with requirement IDs, each row's hash field rendered on a `[data-artifact]` mono node; assert hash N's input references hash N-1 (chain).
3. Stepping transitions a tracked item through accessible states `unproven`, `evidence-attached`, `proven` in that order, assertable via `aria`/`data-state` attributes; the evidence-pending state resolves its color to the `--pending` token (assertion).
4. At the gate step with ≥1 unproven item, the gate node's resolved color equals the `--proven` token value (#34E1A0), copy at first contact reframes the green as the gate holding the line (string assertion), the held item routes to a HANDOFF state whose treatment/label is asserted DISTINCT from COMPLETE, and the DOM contains no node with text `COMPLETE` for the run.
5. The component is keyboard-operable end to end (Tab/Enter/Space advance) with a visible focus ring; assert via keyboard-only driver.
**Depends on:** DS-08, PAGE-05, LOOP-05, CONTENT-13
...
4. §1's first paragraph is the felt-pain antagonist framing and the integrator thesis appears as §1's second paragraph (DOM order assertion that the antagonist precedes the integrator thesis within §1); the HANDOFF bounded-execution/self-correction note is presented as a trust beat in the body flow, not as an error footnote (placement assertion).
**Depends on:** DS-01, CONTENT-06, PAGE-14, CONTENT-11
...
**EARS:** WHERE the /writing route is enabled, the page SHALL render an index of MDX articles ordered by date with title and a human-readable formatted date, and SHALL NOT display a reading-time estimate; the ISO 8601 date SHALL appear only in the `datetime` attribute of the `<time>` element (the visible date being human-readable); IF no articles are published, THEN the page SHALL render an intentional empty state describing the writing's purpose rather than a broken or blank list.
**Acceptance criteria:**
1. When ≥1 article exists, the index lists entries sorted descending by date; assert each entry exposes a title and a human-readable visible date inside a `<time>` element whose `datetime` attribute carries the ISO 8601 value, order is non-increasing by date, and NO reading-time text is present (denylist assertion that no reading-time estimate renders).
...
**EARS:** The /docs page SHALL render an explicit placeholder state communicating that developer documentation is forthcoming, WITHOUT presenting fabricated or dead documentation links, and SHALL include LIVE onward links to /how-it-works, /proof, and /manifesto (the soft onward navigation per PAGE-11) alongside a single soft Get-notified affordance; the placeholder copy SHALL tie the docs' absence to the product's own evidence-generation thesis (docs are themselves an artifact the system will generate and prove, not yet shipped).
**Acceptance criteria:**
1. The page contains a placeholder node whose text communicates docs are forthcoming and ties the absence to the product's evidence-generation thesis (string/presence assertion).
...
3. The page includes live onward links resolving to /how-it-works, /proof, and /manifesto (each href targets a live route in the IA route manifest) alongside exactly one Get-notified capture; assert no pricing, demo, or sales-contact controls in the DOM.
**Depends on:** IA-12, PAGE-11, PAGE-16
...
**EARS:** WHEN a visitor requests a supporting route that is disabled or non-existent, the site SHALL serve a branded not-found/placeholder response with soft navigation back to live pages; the feature-flagged routes /writing and /docs SHALL DEFAULT to flag-OFF at launch (route + nav entry fully absent — no orphan nav link) unless explicitly enabled (in which case they render their intended state, including an enabled-but-empty stub), and SHALL never leave an orphan nav link pointing at an absent route.
**Acceptance criteria:**
...
2. At launch defaults, /writing and /docs are flag-OFF: each flagged route is fully absent (404) AND its nav entry is removed from the rendered navigation; assert no orphan nav link. When a flag is explicitly enabled, the route renders its intended state (or an enabled-but-empty stub) and its nav entry appears.
