...
One header instance persists across SSG/ISR navigations and carries the category descriptor lockup under the wordmark — the "Autonomous SDLC Platform" descriptor — so a deep-link arrival reads the category within a few seconds. A breakpoint switch toggles inline destinations vs a focus-trapped disclosure panel. The descriptor lockup is pinned (or justified) so it never shifts; the bar's scroll-condense behavior either keeps that lockup stable as a discrete informational state change registered in DS-08, or is dropped entirely rather than animating the descriptor out of view — never a decorative loop.
...
not-found and global-error boundaries render branded pages that keep global nav, expose recovery links to canonical destinations, and scrub all stack traces, env detail, and internal identifiers from the 500 surface. The 404 carries the same verbatim governing invariant trust line the footer holds, so even an error surface closes on the product's standard. Neither error page uses alarm-red, yet both read unmistakably as errors within a few seconds through copy and structure rather than color. Correct 404/500 HTTP status codes are asserted.
...
Header, footer, and metadata templates pull gate-verdict/trust copy from CONTENT-13 and any claim values from the single content/claims-registry.json (owned via CONTENT-06); a build-time check rejects any hardcoded "N/N verified" count using the denylist regex. The footer carries the verbatim governing invariant as its single trust line — the byte-for-byte shared constant, not a generic paraphrase — so every page closes on the one sentence the product is held to.
...
Build one ClosedLoop component consuming a `stage` prop ordered intent, decompose, implement, verify, prove, gate — the gate last — and a `mode` (motion | reduced | static). Both the Hero and Closed-loop-explained beats render the same component so the loop never forks visually. In the Hero beat the antagonist kicker is present at first paint above the dominant thesis H1, which the visitor reads in one breath; the powering-stack is demoted out of the hero body to supporting weight and never competes with the thesis. The Hero loop runs cinematically — it is not pre-mounted to the gate, holds no item, and shows no proven accent at rest, so the first green is reserved for the later gate-hold beat. The static SVG labeled diagram is the default render and the permanent fallback; the animated layer mounts on top only when the asset loads. Rationale: a single state contract guarantees HOME-01's fallback and HOME-05's stage progression stay consistent, keeps the loop honest by withholding the gate climax from the hero, and lets HOME-10/HOME-14 force resolved states without component-specific branches.

### D-HOME-03 — Fail-closed gate rendered as a satisfying accent-positive state, never alarm-red  (→ HOME-05, HOME-14)
This climax beat compresses stages one through five at roughly three-to-one and gives the gate about forty percent of the beat as the arrival the descent has been building toward. The held item's fade to --text-faint (decorative/disabled-only) is the primary signal that the line is held; --proven (#34E1A0 on --canvas #0A0B0D, ≈12.5:1 contrast) appears only on a narrow verdict chip, never as a full-cell flood. The held item routes visibly to a HANDOFF affordance that is distinct from COMPLETE, so a block reads as a correction in flight rather than a dead end. The gate is the last stage of the loop. The canonical --error token is explicitly excluded from the gate verdict in all states. Gate-verdict copy is imported from CONTENT-13. The gate-hold follows the LOOP-07 four-beat choreography — approach, evaluate, hold/close, settle — with a measurable success oracle (verdict-chip accent present at the hold frame, no alarm-red across the whole frame sequence). Under reduced motion the static fallback renders the held verdict and gives the gate cell mandatory 2× emphasis so the climax survives without animation. Rationale: framing rejection as the feature requires the rejection to read as system-correct; the fade-primary signal, rationed chip green, and distinct HANDOFF keep the brand claim honest and testable.

### D-HOME-04 — Fragmented-stack consolidation as a deterministic scroll-bound state machine  (→ HOME-03, HOME-14, HOME-15)
The ProblemGrid maps scroll progress to a finite set of discrete consolidation steps via a pure progress->state function (no time or randomness), so any progress value reproduces an identical state and reverse-scroll replays steps in reverse. The grid carries at least twelve real, named tool tiles, visually tiered so the visitor reads two ranks: the wired-in spine tools that the engine actually orchestrates versus the verification and substrate tools it composes around them. A betrayal sub-beat lives inside this section as a quiet single-artifact reveal — one self-report that compiled, passed, and was never connected into a live execution path (grounded verbatim in market-research §1) — with no alarm-red, framed as the visitor's own lived scar rather than fear-mongering. As progress reaches 1.0 the tiles consolidate into one engine while Temporal renders outside that engine, labeled an optional orchestration substrate, so the assembly shows its seams and reads as honest rather than magical. Each consolidated label leads with its human-legible name as the primary text and subordinates the engineering spine string beneath it, and an enabling-primitive line states why this only became assemblable recently — deterministic per-edit enforcement via command-type hooks that fail closed. Each step carries a `data-motion-state` from the DS-08 registry and the component exposes `data-motion-integrity` for the runtime assertion. Rationale: determinism is what makes HOME-15's robustness and HOME-14's no-decoration rule objectively checkable, the seam keeps the consolidation believable, and clean pin-release prevents CLS.
...
Author both beats as fully-formed static DOM first (a real two-column table for the role->substitute pairs; two lists for removes/guarantees), then layer scroll-tied reveal/recede/resolve purely as presentation. The replace-table uses a substitution grammar: the reveal reads as machine-rendered text taking the place of the human-labored text it removes, not as a row lighting up green — the outcome the human stops owning is stated first, the mechanism that now owns it second, and each verifier and auditor cell names its per-edit role. Row and list copy come from the content layer (CONTENT-06). Receded state uses muted/--text-faint; no proven accent appears in the replace-table, so the first green stays reserved for the gate-hold and evidence-record beats. The-leap beat is a static resolved ledger — the two columns (jobs you stop doing, guarantees you gain) read instantly with their before/after contrast carried structurally by the column split and group headers, not by color alone, so it remains legible in grayscale and breaks the motion grammar of the surrounding beats. Rationale: building static-first satisfies HOME-10's "no content trapped in animation" by construction, substitution carries the empathy beat without spending green, the structural ledger contrast keeps the leap color-independent, and accent usage stays within the ≤3–5% budget.
...
The beat opens with the auditor question put to the visitor — could an auditor prove the agent did what it claimed — co-framed for the visitor's own self-interest so the unregulated builder is bitten too: could you, months from now, prove which requirement this code satisfied and that it was ever independently verified. That question, not a comparison table, carries the beat. Beneath it the EvidenceTrace renders exactly one record on the homepage plus a single framing anchor that names what the record is; the append-list of records lives on /proof, not here. The record exposes the four-field schema (test_file, test_name, output_hash, collected_at) inside `[data-artifact]` nodes — the only nodes permitted to use monospace — and is explicitly labeled illustrative sample data. The phrase "governing invariant" is not used as visitor-facing copy in this beat. Illumination to accent is driven by evidence-field completeness — it fires only when all four fields are present — and is decoupled from scroll position, so the proven state reflects real completeness rather than how far the visitor has scrolled. This is the second and final place green appears on the homepage. A shared claims firewall runs the value-agnostic denylist regex `\b\d+\s*/\s*\d+\s+(formally\s+)?(verified|checked)\b` over all homepage copy and metadata at build/test time; all claims resolve through content/claims-registry.json (CONTENT-06). Rationale: leading with the self-administered auditor question converts watched relief into a verdict the visitor renders, confines mono to artifacts, prevents any hardcoded "N/N verified" count from shipping, and ties proof visuals to real completeness.
...
The Who-its-for beat leads with the pain hook — the standard the reader is already held to where "looks done" is unacceptable — before any domain name is spoken, so the highest-stakes reader feels seen by the problem, not by a borrowed industry label, and a quiet no-lock-in micro-line defuses the lock-in fear in the same breath. The Soft-close resolves the Beat-1 antagonist arc and offers only soft actions: "See how it works" as the primary affordance linking to `/how-it-works`, alongside `/manifesto` (routes from IA-03), plus an optional Get-notified control delegated to the PRIV-owned notify component, which renders distinct submitting/success/error/already-subscribed states. No pricing, demo, booking, logo-wall, or sibling-product links appear in Soft-close or Who-its-for. Rationale: pain-before-domain and the no-lock-in micro-line widen the audience honestly, ownership of notify by PRIV centralizes consent/state handling, and forbidding funnel elements keeps the manifesto tone and avoids unverifiable customer claims.
...
One Rive state machine owns all stage progression, gate behavior, and evidence-proving transitions; named inputs/triggers are the host's only write surface. No CSS-timed or Framer Motion animation touches the loop. The centerpiece is split into two layers: a motion layer carrying only the loop geometry, the node labels, and the accent — with no tool names ever appearing inside the moving artboard — and a stationary panel beside it holding the artifact and prose copy. The single state machine drives the loop in two playback modes from the same source of truth: a hero autonomous mode and a Beat-5 scroll-driven mode, never a forked second machine. Rationale: a single deterministic source makes the three acceptance goals (LOOP-03) testable at captured frames, separating motion from a stationary copy panel keeps tool names out of the geometry while one state machine with two playback modes preserves single-source-of-truth, and the runtime motion-integrity assertion (D7) proves every animation maps to a state.
...
The trace is an append-only DOM list; each record exposes test_file, test_name, output_hash, collected_at as navigable text, and each link references the prior record's hash to form a visible chain. The full append-list is the /proof rendering of this panel; the homepage proof beat shows only one record plus a framing anchor (per D-HOME-06), so the lockstep append sequence lives where the auditor-minded reader expects it rather than crowding the keynote descent. Rationale: rendering records as real DOM text (not canvas) satisfies both the tamper-evident reading and the accessibility requirement in one structure, and scoping the append-list to /proof keeps the homepage record singular.
...
Author /manifesto as numbered MDX sections that open antagonist-first: §1 leads with the enemy — the unpaid, invisible proving the human has been forced to become — before the thesis resolves it, then proceeds through fragmented stack, human-as-infrastructure, closed loop, and proof wedge, with the governing invariant stored as a shared constant and asserted byte-for-byte as the single pull-quote. §1 also hosts the integrator thesis and the no-lock-in block. Rationale: single canonical invariant string, reused not retyped.
...
<PoweredBy> is the immediate sequel to the fragmented-stack collapse — the "what's in the box?" reveal that opens the consolidated engine the visitor just watched form — rendered as a collapse-reveal, never a logo-wall. It reads the committed stack manifest and renders each tool as a mono [data-artifact] entry showing the tool name, its B-block ID glossed in plain language rather than left as a bare code, and a one-line pain-point tie; a membership assertion confirms every listed tool exists in the manifest, and DOM/numeric scans confirm no logo <img> and no stat strings. Each entry is backed by a qualitative, citation-backed record in content/claims-registry.json carrying a verifiability_status of design-intent (or, for the Claude Code hooks, emerging/version-gated), with every claim cited to the product spec at .kiro/specs/spec-to-evidence-control/requirements.md and .kiro/specs/spec-to-evidence-control/design.md. The registry covers the official Claude Code hooks; the initializer, implementer, verifier, and research subagents; the git-worktree plus scope-sequencing gate; reasoning-loop detection; requirement-ID propagation via W3C Baggage; the OpenFeature/flagd kill-switch; OWASP ZAP DAST; DeepEval eval-gating; and Hypothesis property tests. Rationale: a claims-integrity-safe substitute for the conventional logo-wall that keeps every Powered-by claim qualitative, grounded in the committed spec, and honest about the hooks' emerging, version-gated status.

### D-CONTENT-19 — No-lock-in block on /how-it-works + manifesto  (→ CONTENT-19)
A shared no-lock-in content block names feature_list.json, EvidenceRecord, and requirement-ID Baggage and states "opinionated defaults, not a cage"; it carries a concrete EvidenceRecord JSON glimpse rendered as a mono [data-artifact] sample beside a plain-language human gloss of what each field means, so the portability promise is shown in the artifact the visitor would actually walk away with, not merely asserted. It is placed after the powered-by reveal and after the proof-model beat, where the visitor has just seen the engine and its evidence; it renders on /how-it-works and in the manifesto, with the claim registered in the claims ledger. Rationale: a concrete record plus gloss makes the portability promise tangible and verifiably grounded, and placing it after powered-by/proof lets it answer the lock-in fear at the moment it arises.
...
A single `(site)` route-group layout defines the shared shell as a fixed set of slots: the navigation with its category descriptor lockup, a visible-on-focus skip-link as the first focusable node, the banner/main/navigation/contentinfo landmarks, the breadcrumb for nested routes, the soft-action footer carrying the verbatim governing invariant, and exactly one Get-notified capture slot owned by the layout (so the single-affordance invariant is structural, not per-page). Centralizing the shell keeps onward navigation, token application, and non-functional budgets uniform across all supporting pages and removes per-page drift. Print styles are NOT part of the shell scope — they are declared per-route (only on /manifesto and /proof) so the shell never ships a print stylesheet to routes that do not want one. Footer onward-nav links resolve against the IA route manifest rather than hardcoded paths.

### D-PAGE-02 — Six-stage loop explainer and B01-B19 taxonomy  (→ PAGE-01, PAGE-02, PAGE-12)
The /how-it-works page composes a six-stage ordered loop explainer and the capability taxonomy, framed as "the same loop at higher resolution." The capability blocks are grouped into clusters that align to the six loop stages (gate last) rather than presented as a flat catalog, and the taxonomy is framed as a reveal — an expansion of the loop the visitor already met — so each cluster reads as one stage opened up rather than a wall of blocks. Stages and clusters are data-driven from a typed manifest so counts and order are structurally guaranteed (six stages, the canonical B01-B19 block set). Block IDs and stage artifacts are the only mono text and carry `[data-artifact]`. Gate-stage framing copy is pulled from CONTENT-13, never inlined.

### D-PAGE-03 — /proof information spine  (→ PAGE-03, PAGE-08)
A fixed four-section spine opens with the auditor test as the challenge — could an auditor independently prove what was claimed — and then answers it with invariant → evidence schema → verifier principle, rendered in deterministic order from an ordered config. A bridge sentence above the audit-trace ties the spine to the interactive trace that follows, so the reader crosses from principle to demonstration without a seam. The evidence schema lists exactly the four canonical fields on `[data-artifact]` nodes. The verifier-principle section states the independent-verifier principle qualitatively — the agent that proves a change is not the agent that wrote it — and frames the gate via the canonical caption: the Stop hook holds locally, an OPA/Conftest zero-evidence policy holds at merge, and the GitHub ruleset makes both required, with the gate read as the last stage of the loop. No quantitative verification figure is composed here; mechanism is described qualitatively and any claim resolves to the claims registry.

### D-PAGE-04 — EvidenceTrace as accessible state machine  (→ PAGE-04, PAGE-05, PAGE-12, PAGE-15)
The sample audit-trace is an explicit finite state machine (loading/ready/error/static plus per-row unproven → evidence-attached → proven and a terminal gate evaluation). State is held in `data-state` attributes that drive both visuals and assertions. Evidence-pending rows bind to the `--pending` token, not green, so an awaiting state never borrows the proven signal; the gate-held verdict binds to the `--proven` token (#34E1A0) and reads as a correct hold, reframed by copy at first contact rather than as an alarm. A held item routes to a HANDOFF state that is visually and semantically distinct from COMPLETE. Keyboard operability, no-JS server-rendered static trace, and reduced-motion parity are first-class branches, not afterthoughts.

### D-PAGE-05 — MDX manifesto with reading aids and allowlist  (→ PAGE-06, PAGE-07, PAGE-14)
The manifesto renders from MDX through a fixed brand-component allowlist (pull-quote, evidence chip, numbered-section heading, blueprint rule); anything outside the allowlist is rejected at build. Numbered headings auto-generate copyable anchors and feed a named-section side-rail driven by an IntersectionObserver — the active section's name, not a percentage progress bar — so the reader always knows where they are by section title. The manifesto's HANDOFF moment is presented as a trust beat: a self-correction the system performs, read as evidence of honesty rather than as an error footnote. Display headings use weights constrained to 300–600.
