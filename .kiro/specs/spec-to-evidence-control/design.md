...
| `SessionStart` | Session begins | load (git status, `claude-progress.txt`, `feature_list.json`) + inject summary into context: **Phase 0**. Integrity compute + `resume_integrity_ok` write via `state_integrity.py`: **Phase 2** *(Reconciliation 2026-06-16: the hash-compute half is a PHASE-2 augmentation — `state_integrity.py` is Phase 2; a reader building the Phase-0 spine should NOT expect `resume_integrity_ok` to be written from day one. Reconciliation 2026-06-15: non-blocking by contract — it only computes/records the flag; the PreToolUse integrity guard enforces it)* | Nothing (informational load + flag write; cannot block) | REQ-STATE-003, REQ-STATE-005 |
...
def main() -> None:
    ...
    result = with_reentrancy_guard(lambda: evaluate_stop(event))
